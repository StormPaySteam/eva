...
function renderCatalog(filter = 'all') {
  ...
  for (const [cat, gridId] of Object.entries(sections)) {
    ...
    if (!grid) continue;
    const items = PRODUCTS.filter(p => p.cat === cat && (filter === 'all' || filter === cat));
...
window.openProduct = function(id) {
  ...
  content.innerHTML = `
    <div class="product-modal-img" style="background:${p.color}"><span>${p.emoji}</span></div>
...
window.doSearch = function() {
  ...
  for (const [cat, gridId] of Object.entries(sections)) {
    const grid = document.getElementById(gridId);
    if (!grid) continue;
    const items = PRODUCTS.filter(p => p.cat === cat && (p.name.toLowerCase().includes(q) || p.desc.toLowerCase().includes(q)));
